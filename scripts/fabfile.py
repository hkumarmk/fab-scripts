...
env.roledefs.update({'all': env.roledefs['oc'] + env.roledefs['cp'] + env.roledefs['st'] + env.roledefs['ct'] + env.roledefs['lb'] + env.roledefs['db'] + env.roledefs['mc'],
  'nonlb': env.roledefs['oc'] + env.roledefs['cp'] + env.roledefs['st'] + env.roledefs['ct'] + env.roledefs['db'] + env.roledefs['mc']
}) 
...
def copy(src,dst,run_type='run'):
  """ Copy arbiterary files to remote machines"""
  print "%s -> %s" % (src,dst)
...
@task
def test(file):
  sudo("/usr/lib/jiocloud/tests/" + file)
...
def runUserData(file,role=None,node=None):
  ...
  if role:
   ...
   log("No nodes specified, exiting")
   return

  execute(putFiles,hosts=nodes,files = {file: '/tmp'})
  execute(runCommand,hosts=nodes,cmd='bash /tmp/' + file + ' -r nonlb -p http://10.1.0.5:3128',run_type="sudo")
...
def checkInitialPapplyRun():
  ...
  with hide('warnings'), settings(warn_only = True):
    while ( attempt < maxAttempts ):
      attempt += 1
      time.sleep(5)
      initial_papply_run = run('ps -efw | grep /var/puppet/modules/rjil/papply || test -e /var/log/papply.log')
...
def runPapply(num_exec=1):
  ...
  with hide('warnings'), settings(warn_only = True):
    ...
    while ( attempt <= num_exec ):
      ...
      if env.upgrade == 'base':
        try:
          papply_out = sudo('/var/puppet/modules/rjil/papply -b')
        except Exception:
          log("Failed runPapply, retrying")
          papply_out = sudo('/var/puppet/modules/rjil/papply -b')
      else:
        try:
          papply_out = sudo('/var/puppet/modules/rjil/papply')
        except Exception:
          log("Failed runPapply, retrying")
#          papply_out = sudo('/var/puppet/modules/rjil/papply')
